...
export function HomeFxView({
  ...
  return (
    <div
      ref={viewRef}
      className="flex-1 overflow-y-auto px-4 pb-32 pt-12"
      data-testid="fx-view"
    >
        ...
        <div className="rounded-full border border-[var(--ep-border)] bg-[#0b0b0b] px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-[var(--ep-text-secondary)]">
          <span
            className={`mr-2 inline-block h-2 w-2 rounded-full ${
              effectsEnabled
                ? "bg-[var(--ep-red)] shadow-[0_0_8px_rgba(255,16,42,0.85)]"
                : "bg-zinc-700"
            }`}
          />
      ...
      <section className="premium-card rounded-3xl p-5">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h3 className="premium-title text-sm font-black text-white">
              Bass Boost / {t("fx.reverb")}
            </h3>
            <p className="mt-1 text-xs leading-relaxed text-[var(--ep-text-muted)]">
      ...
      <section className="mt-4 rounded-3xl border border-[var(--ep-border)] bg-[#080808] p-5">
        <p className="premium-title mb-4 text-sm font-black text-white">
          Salida / Diagnóstico de señal
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          {["Limiter", "Phase", "Crossover"].map((label) => (
            <div
              key={label}
              className="rounded-2xl border border-[var(--ep-border)] bg-[#111] p-3"
            >
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[var(--ep-text-muted)]">
                {label}
              </p>
        ...
        <div className="mt-5 space-y-2">
          <div className="flex justify-between text-[10px] font-black uppercase tracking-[0.14em] text-[var(--ep-text-muted)]">
            <span>Signal</span>
            <span>-3 dB</span>
          </div>

          <div className="flex h-10 items-end gap-1 rounded-xl border border-[var(--ep-border)] bg-black p-2">
            {SIGNAL_METER_BARS.map((height, index) => (
              <span
                key={index}
                className="signal-meter-bar flex-1 rounded-sm"
                style={{
                  height: `${height}%`,
                  opacity: 0.28 + index / 18,
                }}
              />
            ))}
          </div>
      ...
      <p className="mt-5 px-6 text-center text-xs leading-relaxed text-[var(--ep-text-muted)]">
        {t("fx.description")}
      </p>
    </div>
  );
